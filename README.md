...
python tools/backfill_market_scan_probability_history.py --source-database '<VERIFIED_BACKUP_DIR>/runtime.sqlite3' --target-database data/research/market_scan_probability_history.sqlite3 --output-dir data/research/market_scan_probability_history --history-bars 500 --symbol-limit 225
...
python tools/build_market_scan_probability_historical_context.py --artifact data/research/market_scan_probability_replay/<HISTORICAL_REPLAY_ARTIFACT>.json --output-dir data/research/market_scan_probability_replay
...
The formal H5 path is now a separate all-decisions pipeline rather than an upgrade of the older executable-only label. It consumes an exact current source-v3 official decision set—including success, missing and skipped rows—plus operator-pinned licensed execution sessions for D and every fixed D+1..D+6 session. Raw files, source URI/license interval, unadjusted OHLCV/amount, trading state, effective board/ST/listing/delisting rules, corporate-action references, capacity, the frozen full-market cohort and leave-one-out benchmark are all content-bound and replayed. Outcomes separately label `entry_fill`, `exit_executable` and `net_positive`; the action probability is their three-model conditional product over the original decision population. Current-universe qfq history and the older joint-v2 unavailable skeleton remain audit evidence only and can never satisfy this path.

Licensed normalized sessions are installed with `tools/ingest_market_scan_official_execution.py`: run the argument-free, read-only `contract` operation to emit the exact registry/session JSON Schemas, then use `verify` before `ingest`. Schema output is explicitly non-authorizing and does not create a registry pin, token or digest. Verification still requires the out-of-band registry digest, replays every raw-file byte receipt, content-binds the effective instrument rules, installs only a canonical content-addressed file with exclusive atomic creation, and reopens it before reporting success. See `docs/OPERATIONS.md` for the exact invocation. This intake success is only a prerequisite; it cannot self-authorize probability filtering or v6 ranking.

Formal H5 selection is preregistered and requires at least 292 independent mature signal sessions, at least 95% complete labels, two full time-series OOS folds, positive overall and per-fold Brier Skill, valid monotone bins with at least 20 dates each, positive 95% CI lower bounds for both Brier and log-loss improvement, ECE at most 5%, BH-FDR, calibration, 30+30-session drift checks and execution economics. Passing evidence still does nothing by itself: an exact externally pinned filter-authorization artifact and a separately replayed deployment estimator no older than 36 hours are required. The deployment corpus may append only later sessions to the authorized OOS corpus, and a current prediction must cover the new official run's complete decision set. The scheduler performs this maintenance after close; any unavailable or stale link keeps the filter disabled and probabilities typed null.

Probability-aware production ranking is an additional immutable `full-market-score-v6` projection. It never edits `market_scan_result`, v5 scores or historical ranks. For every v5 success row it computes `adjustment = clip((H5 joint probability - reference base rate) * 20, -6, 6)`, adds that bounded value to v5 raw score, clamps to 0..100, then ranks by v6 raw score descending and symbol ascending. Before v6 can affect a new official batch, a post-preregistration Shadow needs at least 60 complete Top100 sessions and must pass costed net-excess CI, first/last 30-session drift, drawdown, turnover, capacity, unresolved-exit, exposure, overlap, PBO and DSR gates; an out-of-band pinned human `promote` control is then mandatory. Publication is stored in separate immutable SQLite tables plus a content-addressed artifact, strictly replayed on restart, exposed through API/UI/Excel with the v5 baseline and adjustment, and disabled by a durable human rollback. No automatic promotion exists.
...
The after-close scheduler still maintains compact legacy H1/H5/H20 outcome artifacts under `data/research/market_scan_probability_outcomes/`. Those fixed-session diagnostics retain their 224/232/262 date floors and never authorize the new all-decisions H5 path. Formal joint maintenance is separate under `data/research/market_scan_joint_execution/`: it validates licensed official sessions, creates source/outcome/OOS/study/authorization/deployment/current-prediction artifacts, and optionally creates Shadow/control/v6-ranking artifacts. Every stage is content-addressed and idempotent. Missing licensed data, an immature D+6 path, an unqualified study, absent exact authorization, stale deployment, absent same-session official source, failed Shadow, missing human promotion or active rollback is a typed non-authorizing state rather than a relaxed fallback.
...
As of the 2026-08-22 local audit, six source archives exist over four distinct official signal dates: legacy-unbound runs 71/77, three same-date current v5 captures on 2026-08-14, and current run 128 on 2026-08-21. This is far below the 292 independent-session floor. More importantly, no out-of-band official execution registry digest, filter-authorization digest or ranking-control digest is configured, and no licensed official execution-session directory exists; therefore zero sessions qualify as formal all-decisions evidence. The UI distinguishes this external-data blocker from capture pending, source accumulation, maturity, selection, authorization, deployment, Shadow, promotion and rollback states. It must not present the filter or v6 ranking as available.

The 2026-08-23 provider audit also checked whether an already installed free adapter could remove that blocker. A live BaoStock 0.9.1 probe returned unadjusted SH/SZ daily OHLCV/amount together with `tradeStatus`, `isST`, turnover, adjustment-factor, IPO and delisting fields; its stock-basic catalog contained 5,548 SH/SZ stocks including 337 delisted records. The same-date catalog exposed no BJ symbols, so it cannot conserve the frozen SH/SZ/BJ decision universe. In addition, the SDK's BSD package license is not proof that its market-data contents carry the required research-use authorization; BaoStock's published disclaimer reserves website-content rights and requires users to verify important information (https://baostock.com/disclaimer), while the BSE states that use of its market information requires an applicable permission from China Securities Index Data Service (https://www.bse.cn/application/guide.html). BaoStock may therefore remain a non-authorizing SH/SZ completeness probe, but it must not be registered as an exchange-licensed redistributor or combined with an unlicensed BJ scrape to manufacture formal evidence. An operator-provided license document and complete BJ delivery remain mandatory inputs to the existing registry boundary.
...
`historical_replay_v1` is a separate backfill cohort for studying whether longer date coverage is useful before real official archives mature. Create and verify a `tools/runtime_data.py backup` bundle first; an ordinary file copy is rejected because it may omit live WAL content. The history command accepts only the bundle's manifest-bound, sidecar-free `runtime.sqlite3`, uses its static `qfq` cache only as a current-universe/anchor source, and re-verifies it before and after acquisition. It supports the compatibility 360-session window and a separately sealed 500-session extension, downloads an SH/SZ/BJ-balanced Tencent qfq sample into a new isolated SQLite database, and requires every accepted symbol to match the complete fixed-session grid. The 360-session contract remains capped at 120 candidates; the 500-session contract permits at most 225 candidates so the resulting replay can remain under its 100,000-row resource bound after the 61-session feature warm-up and H20 label tail. Replay also requires that database to have no WAL/SHM/journal sidecar, opens it with SQLite `mode=ro&immutable=1`, and rejects a main-file fingerprint change. It then reconstructs 11 common OHLCV features using bars no later than D, maps entry and H1/H5/H20 exits to fixed trusted exchange sessions without shifting a missing bar, and models the absolute `net_return_positive` target with registered costs. Neither stage reuses the official cohort or its net-excess target or can populate an official leaderboard probability: current-universe survivorship, missing historical listing/delisting and ST membership, unavailable amount/turnover, unmodelled price-limit tradeability, public-provider provenance, and possible later qfq rebasing remain explicit limitations. Output is immutable research evidence, not production data or an automatic promotion decision.

The historical-context command deep-verifies one complete replay, then publishes a small content-addressed summary beside it. The summary binds the exact full file by filename, byte count, file SHA-256, and internal replay digest; runtime reads verify that binding without repeatedly rebuilding the 26,784-row replay. `GET /api/market-scans/{run_id}/probability-research` and every verified results page expose it only as `historical_context`, so the **历史回放参考** panel remains visible for intraday runs even though their current probability contract is ineligible. The panel shows the selected H1/H5/H20 AUC, Brier Skill, ECE, sample size, and OOS-fold count. It always carries `selection_qualified=false`, `filter_qualified=false`, and `production_ranking_effect=none`. Missing or damaged historical context becomes a local unavailable reference and cannot suppress valid current point-in-time evidence; conversely, historical evidence can never open the current probability filter.

Probability-history manifests use an explicit compatibility matrix: v3 seals the 500-session extension as `attested_v3`, while v2 remains the trusted 360-session `attested_v2` format. V1 is readable only through the legacy reader and is labelled either `legacy_attested_v1` or `legacy_unattested`. A deeply verified transitional attested-v1 manifest may be published as a new v2 content address without overwriting the old file. Unattested v1 remains audit-readable but cannot be upgraded or used as current trusted research evidence; no schema may be relabelled to claim a different history window.

The retained attested 2026-08-11 replay accepted 96 symbols with 279 independent signal dates and 100% fixed-session label coverage. Its reported 5/5/3 sparsest-bin counts, H5 AUC 0.4938, Brier Skill -0.00978 and ECE 0.0980 belong to the superseded feature/label/split-v2 evidence contract. It remains useful for deterministic replay and negative historical context only; it can never authorize the current v4/v3 filter. Under the current split-v3 224/232/262 date floors, 279 dates are numerically sufficient, but a new current-contract evaluation and every authorization gate would still be required. The UI may display these aggregate negative metrics, but no per-stock probability is exported from this legacy study.
...
Artifacts are immutable, content-addressed JSON outside SQLite and retain Top20/50/100/all plus decile summaries, mean/median/positive rate, trend Rank IC, monotonicity, and date-based uncertainty with a three-session moving-block length. The contract also fixes `validation_gap_sessions=3` as the minimum for any future train/test split; it does not thin the current descriptive cohort aggregation. Existing 1/5/20-session legacy probability semantics remain unchanged. A qualifying joint H5 projection may narrow rows, while a separately promoted v6 publication may replace only the displayed score/rank for its exact new run; both preserve the stored v5 baseline and all older history. `GET /api/market-scans/{run_id}/future-range-research` is read-only and returns `generation_status`, artifact identity, optional aggregate `research`, and a pageable `record_page`; `include_research=false` supports light follow-up pages. The lazy **未来区间验证** panel provides D+1/D+2/D+3, group, symbol, and page controls without replacing the leaderboard. XLSX export includes formula-safe future-range evidence and, when active, the v6 rule/digest, v5 baseline rank/score/raw score, H5 adjustment, probability and reference base rate; unavailable numeric cells remain blank rather than inventing zeroes.

### Personal experimental probability view

The full-market page also offers an explicitly opt-in **历史模型概率筛选与实验排序** panel. Choose independent D+1, D+2 or D+5 close-direction models (fixed future qfq close above D close, not costed trading profit), or retain the original H5 costed-return model (D+1 open to D+6 close, **not D+5**). The UI defaults to D+1; existing API/CLI defaults remain H5-compatible. Filter by the selected model probability, market or symbol/name, and sort an independent list while retaining the original production rank. Paging binds applied filters and model/input versions. The direction models have separate training/calibration, immutable source receipts and no independent OOS evaluation; they never borrow H5 validation metrics. This is **not** formal probability authorization or v6 promotion. All four use completed daily bars rather than live intraday prices and exclude missing or out-of-distribution inputs with reasons. The experiment independently verifies its batch on a bounded read lane without waiting for the formal leaderboard's cold-start replay; navigation alone never grants formal authority. Reloading the page disables the experiment again.

Model artifacts, source history and Choice account/data files remain local and are not included in Git. See [personal experimental setup and limits](docs/PERSONAL_EXPERIMENTAL_PROBABILITY.md) and [resumable Choice research collection](docs/CHOICE_RESEARCH.md). Formal filtering, production gates, exports and automated strategies do not consume this experimental view.
