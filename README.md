...
- `full-market-score-v4` is deterministic and trend-led. Moving-average distance and slope use bounded continuous contributions with a neutral band, and the aggregate trend score is soft-clipped around 50 so finite rules do not saturate at 0/100. Data quality is penalty-only; it never creates strength. A bounded medium-term refinement of less than 0.05 breaks otherwise equal base scores at six-decimal precision, followed only by the stock symbol for deterministic ordering. Change, volume, turnover, and amount are not reused as hidden tie-breakers. Ranking admission now rejects malformed OHLC/liquidity fields and a reported daily return that differs materially from the value derived from price and previous close. Cache origin alone is quality-neutral, while fallback, staleness, malformed fields, K-line anomalies, conflicting same-day bars, quote/K-line mismatch, future quote timestamps, and uncertain single-price execution remain explicit penalties or eligibility failures. The canonical specification and every replay input/component are persisted with a stable SHA-256 hash. The temporal mode contract is part of that fingerprint, so otherwise identical `official`, `intraday`, and `preopen` runs have different rule versions.
...
Every newly published `official` full-market run atomically enqueues a point-in-time capture in the same SQLite transaction as publication. The single runtime leader drains that durable outbox through bounded leases, exponential retry, restart reconciliation, and canonical-latest checks; `preopen`, intraday, TOP100, superseded, incomplete, or unpublished runs never become official training evidence. A successful capture retains the complete registered feature vector, source-evidence digest, cohort identity, and label metadata in a compact immutable gzip JSON file under `data/research/market_scan_probability_source/`. The archive is independent of bounded runtime scan retention, while a capture failure remains observable without changing the already published rank.

The after-close scheduler now maintains compact, immutable outcome artifacts under `data/research/market_scan_probability_outcomes/`. For each archived D snapshot, H1/H5/H20 use the trusted exchange calendar's fixed D+1-open entry and D+2/D+6/D+21 close exits. A target that has not closed is `not_mature`; after maturity, a missing fixed-session bar remains `data_unavailable` and is never shifted to a later convenient date. Repeated maintenance compares semantic state before publication, so unchanged evidence does not create an unbounded stream of duplicate artifacts. The source-research projection reports archived, mature, available, eligible, coverage, next-maturity, maintenance, and fit state separately. The registered H1/H5/H20 evidence floors remain 222/230/260 independent signal dates with at least 95% label coverage. Only after all three horizons have at least 260 available sessions does maintenance run a conservative bounded `sampled_oos_assessment`: it joins canonical sources to their latest verified outcomes, keeps the newest 300 sessions, deterministically samples at most 90 SH/SZ/BJ-balanced stocks per session (at most 27,000 rows), and performs the research builder's replay with `include_records=false`. The compact content-addressed artifact under `data/research/market_scan_probability_fit/` binds the input source/outcome pairs and retains cohort/horizon evidence, research digest and replay status without copying feature vectors or per-symbol forecasts. Because its sampled benchmark is not the registered full-market benchmark, `fit_selection_qualified=false` with reason `sampled_market_benchmark_not_full_market_contract`. The leaderboard remains `status=insufficient_data`, `probability=null`, `selection_qualified=false`, `pipeline_stage=sampled_fit_assessed`, and `selection_status=projection_pending`; `fit_status=sampled_oos_assessment` describes only that bounded diagnostic. No floor is lowered and no assessment becomes a selectable percentage until a separately verified full-market per-symbol projection is published.

The current live corpus still contains only the `2026-08-11` run `#71`: 5,499 verified point-in-time observations across one independent signal date. As of `2026-08-12`, its fixed H1/H5/H20 exits are all still immature. It therefore has no trustworthy H1/H5/H20 probability; every estimate remains `probability=null`. Before any archive exists the UI says **尚未生成研究证据**; an archive without mature labels is described as **点时样本积累中**; a completed but underpowered study says **研究已生成·样本不足**. None of these states authorizes probability filtering.
...
The retained attested 2026-08-11 replay accepted 96 symbols with 279 independent signal dates and 100% fixed-session label coverage, so H1/H5/H20 all crossed their 222/230/260 total-date floors. They still remain `insufficient_data` with `probability=null`: the sparsest H1/H5/H20 calibration bins cover only 5/5/3 independent dates against the registered 20-date per-bin floor. H5 OOS AUC was 0.4938, Brier Skill was -0.00978, ECE was 0.0980, bins were non-monotonic, and the highest bin did not beat the base rate. The result demonstrates that adding dates alone does not make the current 11-feature OHLCV model predictive; the registered bin and calibration gates remain intact.
