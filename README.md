...
- `full-market-score-v3` is a deterministic short-term-strength score: trend contributes through one dedicated profile and data quality contributes 15%, without counting change, volume, turnover, or amount a second time. Cache origin alone is quality-neutral; fallback, staleness, malformed fields, and K-line anomalies remain penalties. The unrounded score is persisted and used before other stable tie-break fields, so integer display rounding cannot invert ranks. Weights, thresholds, component algorithms, rounding mode, metric precision, tag rules, and tie-break order are serialized into a canonical specification with a stable SHA-256 hash. Every successful row stores its inputs and component breakdown, so score and rank can be replayed without provider access.
- Runs and per-stock results are persisted with as-of/data dates, sources, metrics, structured degradation provenance, status, rank, coverage, and duration. The completed `data_date` is the frozen end-of-day snapshot boundary, so later revisions from that same market date remain eligible while another date does not. Active runs are de-duplicated, can be cancelled, and become `interrupted` rather than falsely remaining active after a restart. One repository-generated retry plan controls validation and atomic derived-run creation: the original stays immutable; a failed or interrupted batch is fully recalculated because its partial snapshot was never published, while a published degraded batch may reuse clean rows and recalculate only unresolved rows. Retries reject changed scan contracts instead of mixing scoring rules.
- A run is publishable only when global and SH/SZ/BJ success coverage over eligible rows reaches 95%, at least 90% of each market remains eligible, each market's quote timestamps are parseable and span no more than 15 minutes, and no systemic same-day K-line lag cluster is detected. Deterministically ineligible rows such as suspended or too-short-history stocks remain visible but do not dilute the eligible coverage denominator; the separate 90% guard prevents a systemic data problem from being hidden as mass skipping. A 30-minute whole-run wall-clock budget and repeated completed-trading-date checks stop stale work while leaving unresolved rows available for retry.
