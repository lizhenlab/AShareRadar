...
- Offline evaluation starts from frozen published ranks, separates mode/scope/rule-version cohorts, deduplicates repeated same-session scans, and never mutates production weights. The current evidence is intentionally conservative: see the [2026 evaluation summary](docs/research/FULL_MARKET_SELECTION_EVALUATION_2026.md) and [machine-readable report](docs/research/FULL_MARKET_SELECTION_EVALUATION_2026.json).
...
The evaluator reads published frozen ranks and only later persisted complete trading days. It reports Top 20/50/100 forward returns, positive rate, equal-weight excess return, adverse excursion, rank-band monotonicity, stability, and turnover by mode, market, scan-time regime, and quality bucket. Cohorts below the configured floor are `insufficient_data`; reports never rewrite score weights or a historical `rule_version`.
