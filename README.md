...
The full-market page also offers an explicitly opt-in **历史模型概率筛选与实验排序** panel. It uses a separately built historical H5 model for personal exploration: filter by model probability, market or symbol/name, and sort an independent list while retaining the original production rank. It is **not** formal probability authorization or v6 promotion. The model remains unvalidated for production, uses completed daily bars rather than live intraday prices, and excludes missing or out-of-distribution inputs. Reloading the page disables the experiment again.
