...
class StockEventSummaryTests(unittest.TestCase):
    ...
    def test_external_checklist_events_are_triggered_by_large_move_and_turnover(self) -> None:
        ...
        self.assertIn("龙虎榜", categories)
        self.assertIn("公告", categories)
        self.assertIn("融资融券", categories)
        self.assertTrue(any("龙虎榜席位" in step for step in summary.next_steps))
        self.assertTrue(any("融资融券余额" in step for step in summary.next_steps))

    def test_external_event_rule_order_is_explicit(self) -> None:
        self.assertEqual(
            [rule.name for rule in EXTERNAL_EVENT_RULES],
            ["lhb_candidate", "announcement_check", "margin_check"],
        )

    def test_high_risk_triggers_announcement_event_and_matching_next_step(self) -> None:
        ...
        self.assertIn("公告", [item.category for item in summary.events])
        self.assertTrue(any("核查交易所公告" in step for step in summary.next_steps))
