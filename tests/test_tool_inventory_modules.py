...
def test_python_dependency_layers_keep_runtime_and_engineering_tools_separate() -> None:
    ...
    security = (ROOT / "requirements-security.txt").read_text(encoding="utf-8").casefold()
    ...
    security_lock = (ROOT / "requirements-security-lock.txt").read_text(encoding="utf-8").casefold()
    engineering_tools = {"mypy", "pyflakes", "pytest", "pytest-cov", "ruff"}
    security_tools = {"cyclonedx-bom", "pip-audit"}
    ...
    assert set(re.findall(r"^([a-z0-9-]+)==", security, flags=re.MULTILINE)) == security_tools
    ...
    assert "autogenerated by pip-compile with python 3.12" in security_lock
    runtime_command = next(line for line in runtime_lock.splitlines()[:8] if line.startswith("#    pip-compile "))
    development_command = next(
        line for line in development_lock.splitlines()[:8] if line.startswith("#    pip-compile ")
    )
    security_command = next(
        line for line in security_lock.splitlines()[:8] if line.startswith("#    pip-compile ")
    )
    assert "--generate-hashes" in runtime_command
    assert "--output-file=requirements-lock.txt" in runtime_command
    assert runtime_command.endswith(" requirements.txt")
    assert "--allow-unsafe" in development_command
    assert "--generate-hashes" in development_command
    assert "--output-file=requirements-dev-lock.txt" in development_command
    assert development_command.endswith(" requirements-dev.txt")
    assert "--allow-unsafe" in security_command
    assert "--generate-hashes" in security_command
    assert "--output-file=requirements-security-lock.txt" in security_command
    assert security_command.endswith(" requirements-security.txt")
    ...
    assert security_tools <= set(re.findall(r"^([a-z0-9-]+)==", security_lock, flags=re.MULTILINE))
    assert "futu-api==" not in security_lock
