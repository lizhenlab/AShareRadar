...
29. Observe a disposable scan from fewer than 20 rows through completion. Confirm ETA says `估算中` initially, then becomes bounded; stages and SH/SZ/BJ counters advance, and a simulated provider/coverage/span/distribution failure renders the documented action without publishing a partial official leaderboard.
...
Stock-search requests are not part of the four-request stock-switch baseline: only a debounced, uncached, non-complete user query may trigger one. A selected suggestion then follows the ordinary stock-load budget. The latest recorded desktop/mobile browser matrix is **55 passed, 15 skipped**; the skipped cases are intentional device-exclusive scenarios recorded by Playwright rather than treated as passes.
