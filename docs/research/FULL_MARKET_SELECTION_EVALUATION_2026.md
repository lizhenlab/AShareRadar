...
> 2026-08-13 信任边界更新：下文数值是历史只读研究结果，不会因新封印而重标。当前运行时以 `market-scan-snapshot-digest-v2` 封印全部公开持久化 run/result 字段，以 `publication|legacy_backfill` 区分证据来源；动作还必须通过精确全市场范围与 diagnostics-v1 门禁：无 blocker、唯一 info `score_distribution.pass`、无任何评分分布冲突。PIT v4 进一步绑定 `quote event <= quote_observed_at <= decision` 与每根 bar 的 `as_of`。旧 evidence、legacy backfill 或诊断冲突发布仍可 audit/read，但不能晋级或授权。托管 probability/source/outcome/fit/future/individual 产物与 retention 共享跨进程租约并按批次全有或全无；大 probability 文件以有界内存校验，DB-only restore 在侧产物存在时拒绝。
