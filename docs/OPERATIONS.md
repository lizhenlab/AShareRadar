...
The legacy deployment-refit and joint-v2 contracts remain deliberately non-authorizing for executable-only evidence. The formal H5 maintenance path requires the four `ASHARE_RADAR_MARKET_SCAN_OFFICIAL_EXECUTION_*` settings below, including an out-of-band pinned registry digest, licensed raw deliveries and normalized session artifacts. It joins each action-eligible source-v3 official decision set to D and fixed D+1..D+6 sessions, verifies all raw/provenance/trading-rule/corporate-action/capacity/benchmark bindings, and labels entry fill, executable exit and positive net return over every original decision. Public qfq history is never upgraded. The scheduler and startup warm-up replay this path only when the pinned registry digest is configured.

Before mapping a licensed supplier delivery, print the exact validation schemas. This operation has no store arguments, performs no network or filesystem writes, and cannot create an authority token or independently pinned digest:

```bash
PYTHONNOUSERSITE=1 .venv/bin/python tools/ingest_market_scan_official_execution.py contract
```

Do not copy a normalized session directly into the managed directory. First place the licensed raw bytes under the configured raw root, place the operator-reviewed registry at its configured path, and obtain its SHA-256 through an independent channel. Then use the intake command below. `verify` performs the complete registry/session/raw-byte replay without writing; `ingest` repeats that verification, publishes the canonical session under a content-addressed filename with exclusive atomic creation, reopens the managed file through the same trust boundary, and reports the resulting catalog. The candidate and raw files are never edited or deleted. Effective instrument rules carry their own recomputed content digest in addition to the enclosing trading-state, row and artifact digests.

```bash
PYTHONNOUSERSITE=1 .venv/bin/python tools/ingest_market_scan_official_execution.py \
  --registry-path data/research/market_scan_official_execution/source-registry.json \
  --registry-digest "$ASHARE_RADAR_MARKET_SCAN_OFFICIAL_EXECUTION_REGISTRY_DIGEST" \
  --raw-root data/research/market_scan_official_execution_raw \
  --session-directory data/research/market_scan_official_execution/sessions \
  verify --candidate <normalized-session.json>
...
PYTHONNOUSERSITE=1 .venv/bin/python tools/ingest_market_scan_official_execution.py \
  ...
  --raw-root data/research/market_scan_official_execution_raw \
  --session-directory data/research/market_scan_official_execution/sessions \
  ingest --candidate <normalized-session.json>
```

Exit `0` with `status=ready` proves only that the licensed intake catalog is mechanically valid. It does not prove H5 maturity, selection, authorization, deployment, current prediction, Shadow qualification or production promotion. After changing runtime environment pins, restart the service so the in-process settings snapshot uses the reviewed values; the next startup/scheduler maintenance pass will replay every downstream stage.

Selection uses the preregistered H5 three-component model and requires 292 independent mature signal sessions plus every proper-score, calibration, FDR, drift and economics gate. After it passes, install the independently reviewed exact filter authorization at `ASHARE_RADAR_MARKET_SCAN_JOINT_EXECUTION_AUTHORIZATION_PATH` and pin its digest with `..._AUTHORIZATION_DIGEST`. A deployment is then fitted from the exact authorized corpus plus only later sessions and expires after 36 hours; a same-session official source strictly after its training cutoff is required for current predictions. Never copy a test token, edit a content-addressed file, reuse a final OOS fold, lower the session floor or make the deployment lifetime longer to open the UI.

Production ranking remains v5 until an additional `full-market-score-v6` Shadow passes at least 60 post-preregistration complete Top100 sessions and all costed excess, CI, 30+30 drift, drawdown, turnover, capacity, unresolved-exit, exposure, overlap, PBO and DSR gates. A human reviewer must then supply a separately pinned `promote` control using `ASHARE_RADAR_MARKET_SCAN_PROBABILITY_RANKING_CONTROL_PATH` and `..._DIGEST`; only newer official runs can receive v6. The projection adds a bounded `clip((p-base_rate)*20,-6,6)` adjustment to v5 raw score and stores the result in separate immutable tables/artifacts. API, UI and Excel show both v6 and the unchanged v5 baseline. To disable it, pin a valid human `rollback` control bound to the promotion/publication; the durable rollback remains effective after restart. There is no automatic promotion or history rewrite.
...
Legacy source, outcome and fit-assessment archives are evidence, not authorization to display or filter on a probability. Before assessment, the read projection reports `pipeline_stage`, `next_maturity_date`, `maintenance_status`, `fit_status`, `selection_status`, and per-horizon archived/mature/available/eligible observations and sessions. A bounded legacy assessment still cannot authorize the formal all-decisions path. Inspect `official_execution_evidence` and `joint_execution_evidence` for the new stages: registry configuration, verified licensed sessions, mature H5 sessions versus 292, selection, exact authorization, deployment, current-prediction count, ranking Shadow, human control, v6 run/count, blockers and failures. Do not lower a floor, duplicate one date, infer a label from archive time, replace an unavailable fixed session, or describe an unverified study as selectable.
