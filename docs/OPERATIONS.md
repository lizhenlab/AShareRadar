...
The grouped-date evaluator fits every complete non-overlapping test fold independently, drops a partial trailing test window, aggregates all fold-tagged OOS predictions, and records each fold's split/artifacts/base rate/digests. Brier Skill uses the corresponding calibration-period base rate; forecasts after the final test window use only the final complete fold. Because H-session outcomes overlap, confidence intervals use deterministic circular moving-session blocks of length `max(1, H)` rather than treating adjacent dates as independent. The 5-session equal-weight-market net-excess target is primary; 1 and 20 sessions and absolute net return remain separately auditable. Label v2 buys at the first tradeable-session open after the scan, counts that session as holding day 0, and sells at the H-th later tradeable-session close with T+1, capacity, locked-limit/suspension and round-trip cost rules; no missing or locked exit is shifted to a more convenient date. An unavailable or degraded effective-dated rule profile produces non-modelled evidence and is excluded from both the cohort benchmark and training. A verified profile may retain `daily_bar_model_limited=true` for ordinary OHLC-only intraday uncertainty under the registered open/close model; this flag is a limitation, not proof of exact fill order. Platt remains the display calibrator, the empirical-Bayes result remains a baseline, and Isotonic stays a sample-gated comparison candidate whose status/parameters/metrics are recorded without automatic selection. Calibration and selection authorization are separate states: a study may display a verified calibrated estimate while remaining unusable as a filter. Selection additionally requires the complete bound label/cost contract, positive aggregate OOS Brier Skill, effective monotonic probability bins whose highest bin beats the base rate, at least two complete OOS folds, and positive Brier Skill in every complete fold.

Artifacts use canonical finite JSON, exact schema/version contracts and a SHA-256 payload integrity seal. The seal detects alteration but is not a digital signature, signer identity or authenticity proof. Before reporting success, the CLI loads the complete artifact manifest, rejects missing/duplicate artifacts and records, regroups full feature/label inputs by cohort, and independently refits every target/horizon. Only exact reconstructed study fields and input/evidence digests yield `full_input_replay_verified=true` plus an `artifact_set_replay` breakdown; otherwise the command fails instead of publishing a credible flag. Stdout is one `market-scan-probability-evaluation-summary-v1` JSON object. `--report` atomically writes the identical object, refuses to overwrite/link to SQLite or an artifact, and is suitable for machines rather than a promotion authorization. `GET /api/market-scans/{run_id}/probability-research` and the ordinary results response load only verified artifacts and never recompute a model or write SQLite. `probability_horizon=1|5|20` selects the displayed/filter horizon; `min_upside_probability=0..1` returns `422` unless the primary-target evidence is both `calibrated_shadow` and replay-consistent `selection_qualified=true`, and a null estimate is never treated as zero. These values are Shadow-only and do not affect production order. If database persistence is later desired, implement it as a separately invoked, reviewed importer into a dedicated versioned table; do not let this evaluator or the request path mutate the runtime database.

Probability data accumulation has two durable, deliberately separate artifact stages:

- Publishing a canonical `official` full-market `success` or `degraded` run inserts its capture request into `market_scan_probability_capture_outbox` in the same SQLite transaction. The single runtime leader reconciles missed eligible rows at startup, claims bounded leases, recovers expired/interrupted claims, retries failures with bounded backoff, and rechecks canonical-latest identity before capture. `preopen`, intraday, TOP100, failed/unpublished, incomplete, and superseded same-date cohort runs are skipped or rejected as appropriate. A capture failure remains visible in outbox/monitor evidence but cannot downgrade the already published rank. Each file under `data/research/market_scan_probability_source/` is deterministic gzip, immutable, content-addressed, restart-verifiable, and independent of ordinary runtime scan cleanup.
...
- H1/H5/H20 retain their 222/230/260-session and 95%-coverage gates. Only after all three horizons have at least 260 available sessions does the same maintenance pass run a bounded `sampled_oos_assessment`: it joins canonical source/latest-outcome pairs, retains the newest 300 sessions, deterministically samples at most 90 SH/SZ/BJ-balanced stocks per session (27,000 rows), and invokes research replay with `include_records=false`. It writes a compact content-addressed gzip summary under `data/research/market_scan_probability_fit/`; the artifact contains cohort/horizon evidence plus bound input-pair/research digests and replay status, but no feature-vector copies or per-symbol forecasts.

Source, outcome and fit-assessment archives are evidence, not authorization to display or filter on a probability. Before assessment, the read projection reports `pipeline_stage`, `next_maturity_date`, `maintenance_status`, `fit_status`, `selection_status`, and per-horizon archived/mature/available/eligible observations and sessions. A bounded assessment exposes `fit_status=sampled_oos_assessment`; because its sampled benchmark is not the registered full-market benchmark, `fit_selection_qualified=false` and `fit_selection_qualification.reason=sampled_market_benchmark_not_full_market_contract` are invariant. Since no full-market per-symbol projection is published, the read projection still reports `status=insufficient_data`, `probability=null`, `selection_qualified=false`, `pipeline_stage=sampled_fit_assessed`, and `selection_status=projection_pending`. Do not lower the date or coverage floors, duplicate one date, infer a label from archive time, replace an unavailable fixed date, or describe the sampled assessment as a selectable `calibrated_shadow` projection.

The first controlled real capture retained run `#71` for `2026-08-11`: 5,499 of 5,499 successful rows, 1,953,891 compressed bytes and 17,512,315 canonical uncompressed bytes, with restart load and digest verification succeeding in about 18.1 seconds. That is still only one independent live cohort date. As of `2026-08-12`, its H1/H5/H20 fixed exits are all immature; the horizons therefore remain `insufficient_data` with `probability=null`. This evidence proves capture completeness, not predictive validity. At the observed compressed size, 244 similar sessions would consume roughly 455 MiB, but feature mix and compression can vary, so monitor the directory rather than treating that estimate as a quota.
...
The commands above record the retained attested 2026-08-11 evidence and must not be rerun against the same immutable target; choose a new target name for a genuinely new provider vintage. A 90-symbol probe failed closed after about 19.2 seconds because accepted Beijing coverage fell below 20 and published no target. The verified-backup 120-symbol run completed in about 28.6 seconds and accepted 96 symbols (SH 37, SZ 32, BJ 27); 13 new/short histories and 11 fixed-session-gap series were excluded. Its source-backup SHA-256 is `d3d746b2914d48a0f7a338d34607dd4a2970e18fd0903612f696e687f4875464`, with the same manifest-bound backup fully verified before and after acquisition. The history database contains 34,560 rows, exactly 360 sessions per accepted symbol from 2025-02-20 through 2026-08-11, is 8,847,360 bytes, and has SHA-256 `946db7db1a5b62342d139f223784545cd71020ed87a5679eb7d3a7a3fa80927a`. Its current v2 manifest digest is `b30942ef71d64b45642adbd61df329f3ceb687917dd0cdf0451c9386517d3b3e`; the earlier `abe35f6523…` manifest is retained as `legacy_attested_v1`, while `7fe6164b4b…` is `legacy_unattested`. Use the default reader only for current attested v2. Use the explicit legacy reader for audit, and upgrade only a deeply verified `legacy_attested_v1`; never infer attestation for the unattested form or overwrite either old file. The generated replay window is 2025-05-21 through 2026-07-13. The final immutable/sidecar-attested replay completed in about 87.2 seconds, retained 26,784 date-symbol records (96 × 279), achieved 100% fixed-session label coverage for every horizon, and wrote a 68,553,345-byte artifact with digest `045a11cbb120f8ac82290af30f107818d3f94fcd57adeaf09845d2792943f853`. The query-only source fingerprint did not change. Earlier pre-attestation replay files, including the intermediate `118fc869…` result, are retained only as legacy local evidence and are not the current verified result.

Date sufficiency passed, but predictive evidence did not:
...
Each fit had 120 training, 40 calibration, and 60 test sessions. The remaining registered blocker is `minimum_probability_bin_sessions`: although every horizon has 279 total signal dates, the sparsest H1/H5/H20 calibration bins contain only 5/5/3 independent dates, below the 20-date per-bin floor. The near-random or worse AUC, negative Brier Skill, H5/H20 calibration error, and failure of the highest bin to beat the base rate are independent reasons not to weaken that gate. This backfill answers the total data-volume question but does not validate the 11-feature OHLCV probability model and must not be shown as an official 5-day probability.

Source, outcome, fit-assessment, history, and replay archives are not automatically pruned. Source files may be the only surviving point-in-time evidence after SQLite run retention and should be backed up before any manual deletion. Outcomes and fit summaries are compact but remain integrity-bound to their exact source/outcome digests and must be retained with that evidence chain. History/replay files are reproducible only relative to the exact recorded provider and qfq vintage, so keep each SQLite file, content-addressed manifest, replay artifact, and source fingerprint together. Review `du -sh data/research/market_scan_probability_source data/research/market_scan_probability_outcomes data/research/market_scan_probability_fit data/research/market_scan_probability_history data/research/market_scan_probability_replay` periodically and place large exploratory output on a separately monitored volume when necessary. Never alias source/target/output paths, point a manifest/report at SQLite or an existing artifact, or overwrite a published bundle.

Within one running process, the source-research reader reuses compact summaries for unchanged file fingerprints and extends cumulative cohort evidence with an O(N) hash chain; it does not retain an O(N²) list for every run. That is a warm-process optimization only. After restart, the first probability-research request must fully decompress and deep-verify every source plus selected outcome/fit archive before rebuilding the memory cache. With 230 real full-market sources this may take minutes, and the 230-small-fixture regression is not a latency benchmark. There is no persistent verified summary catalog in this release because an unbound sidecar would weaken payload integrity. Plan first-read latency during maintenance/restart and do not claim that cold-start scaling has been solved.
...
The wrapper always contains `generation_status`, `artifact`, `research`, and `record_page`. Missing artifacts return `not_generated` with empty records; a verified underpowered artifact returns `insufficient_data`; non-published/non-official runs—including `preopen`—return `422`; a current-schema integrity failure returns `409`. The browser panel loads lazily only for the selected official batch and displays an unavailable explanation for other modes. Ordinary XLSX export appends **未来区间验证** from the same verified artifact, including fixed-session status, exploratory range/path evidence, executable gross/net/net-excess evidence, versions/digests, and blank cells for unavailable values. Neither API nor export invokes providers, recomputes outcomes, writes SQLite, or promotes a model.
...
Active/manual work suppresses automatic overlap, and a same-day manual cancellation is not overridden automatically. Run snapshots are retained with their results; the configured run count is a convergence target with active/retained-retry ancestry safety exceptions, as described under Retention Cleanup. Full-market scoring never calls the LLM.
...
Saved discovery presets are local SQLite user data. Applying one queries only a completed persisted run and does not contact providers. The supported criteria are market, industry, ST/new-stock state, quality, trend, change, turnover, amount, and score. Research enqueue records source run/preset/revision/name, and later scan retention does not invalidate that provenance. Import/export uses a versioned checksum. Rank movement is available only against the immediately previous completed run with the same rule version.
