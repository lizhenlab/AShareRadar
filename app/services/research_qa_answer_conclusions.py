...
def _t_strategy_invalidations(context: InvalidationContext) -> list[str]:
    return dedupe([*context.t_strategy.stop_conditions, f"跌破支撑 {_level(context, 'support', 'support_available')} 或冲高回落放量。"])


def _buy_or_short_term_invalidations(context: InvalidationContext) -> list[str]:
    return dedupe([*_first_clean_items(context.evidence_chain.invalidations, 3), *_first_clean_items(context.diagnosis.hard_risks, 2), f"价格有效跌破 {_level(context, 'support', 'support_available')}。"])


def _sell_invalidations(context: InvalidationContext) -> list[str]:
    return dedupe([
        f"放量站稳压力 {_level(context, 'resistance', 'resistance_available')} 后，卖点需要重新评估。",
...
def _risk_invalidations(context: InvalidationContext) -> list[str]:
    invalidations = [
        ...
        if getattr(item, "score_available", True) and _sort_score(item.score) >= 42
    ]
...
def _risk_reward_invalidations(context: InvalidationContext) -> list[str]:
    return dedupe([
        ...
        f"有效跌破支撑 {_level(context, 'support', 'support_available')}，说明下方风险开始兑现。",
...
def _theme_invalidations(context: InvalidationContext) -> list[str]:
    theme_invalidations = [
        ...
        f"跌破关键支撑 {_level(context, 'support', 'support_available')} 后，题材解释不能替代价格纪律。",
    ]
...
def _level(context: InvalidationContext, value_field: str, availability_field: str) -> str:
